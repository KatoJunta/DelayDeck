...
**DelayDeck** は、OBS Studio の標準機能である「配信遅延」をより柔軟に使えるようにするプラグインおよび Relay Engine です。

標準機能では配信中に遅延の有効/無効を切り替えられません。特に、時差のないリアルタイム対戦ゲームを配信する場合、視聴者側の情報が先に届いて不利になることがあります。DelayDeck は、そのような場面向けに開発されました。
...
- 配信中の **遅延配信 ON/OFF**
- 遅延切替・ライブ復帰時の **スレートシーン** 表示
- 未送出遅延コンテンツの **即時リアルタイム復帰**（Dump Buffer、確認ダイアログ付き）
- 配信開始前の **preflight チェック**（Relay の状態、OBS 配信先、スレートシーン設定など）
- YouTube / Twitch などの **配信先プリセット**
...
OBS Studio はバージョンによってプラグインが起動しないことがあります。**必ず、利用中の OBS バージョンに合った ZIP をダウンロードしてください。**

新しい OBS メジャーバージョンが出たあと、対応ビルドの用意には手動作業が必要です。目的のバージョンが Release にない場合は、[X（@KatoJunta）](https://x.com/KatoJunta) または [GitHub Issues](https://github.com/KatoJunta/DelayDeck/issues) からご連絡ください。
...
これで導入は完了です。テスト配信で動作を確認してから本番配信に使ってください。
...
### ストリームキーを入力したけど、設定画面に表示されない。保存されていない？

**保存されています。** セキュリティのため、ストリームキーは設定画面や OBS のプロファイルに **平文では保存しません**。

Windows では次の順で保存を試みます。
...
DelayDeck は次の方針で重要な情報を扱います。

- ストリームキーをログや診断情報に出さない
- Relay との通信には、起動のたびに生成される **セッショントークン** を使う（localhost のみ）
- Relay API は原則 `127.0.0.1` に bind し、LAN へ公開しない

ただし、PC 自体への不正アクセスやマルウェアからは、他のアプリと同様に保護できません。PC のセキュリティ更新と、信頼できるソフトウェアのみの利用をおすすめします。

### DelayDeck を経由すると、配信や PC のパフォーマンスは落ちる？

**多少のオーバーヘッドはありますが、通常は OBS のエンコード負荷の方が大きいです。**

DelayDeck の構成は次のとおりです。

- **映像のエンコード** … OBS が担当（従来どおり）
- **遅延バッファと送出タイミング** … Relay が担当
- **OBS → Relay** … 同一 PC 内の localhost RTMP（`127.0.0.1:9401`）

localhost 経由のため、ネットワーク帯域への影響は小さいです。一方で、Relay が遅延分のメディアをメモリ上に保持するため、**遅延秒数が長いほどメモリ使用量は増えます**。CPU も遅延秒数に応じて追加で使用します。
...
### OBS の配信先が `rtmp://127.0.0.1:9401/live` になるのはなぜ？
...
v0.1.0 時点では、**数時間以上の長時間配信** については、開発環境での十分な検証ができていません。

DelayDeck を使って **長時間の配信（目安: 2 時間以上）** に成功した場合は、ぜひ開発者へご連絡ください。気軽に報告する場合は [X（@KatoJunta）](https://x.com/KatoJunta) がおすすめです。詳細な報告や公開での記録に残したい場合は [GitHub Issues](https://github.com/KatoJunta/DelayDeck/issues) も使えます。
...
Issue、[X（@KatoJunta）](https://x.com/KatoJunta) での要望・協力も歓迎します。
...
- [配信者向け操作の説明](docs/operator-controls.md)
- [English README](README.en.md)
