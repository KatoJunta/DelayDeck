...
**配信中に配信遅延をオン/オフできる OBS Studio 向けプラグイン**
...
&nbsp;·&nbsp;
<a href="https://x.com/KatoJunta/status/2064341479984947235">デモ動画</a>
&nbsp;·&nbsp;
...
時差のないリアルタイム対戦ゲームを配信する場合、悪意のあるストリームスナイパーやゴースティングをしようとする人物に対して、配信者が不利になることがあります。標準機能では配信中に遅延の有効/無効を切り替えられないため、配信者はゲームの成績を優先するか、視聴者との交流を優先するかを選ぶ必要がありました。

DelayDeck は、**配信を中断することなく遅延の ON/OFF を切り替えられる**仕組みを提供し、ゲームプレイと視聴者との交流の両方を楽しめるようにすることを目的に開発されました。
...
| 🖼️ **スレートシーン**       | 遅延切替・ライブ復帰時に視聴者向け画面を表示                    |
...
> [!WARNING]
> OBS Studio のバージョンに違いがあると、プラグインが正常に読み込まれないことがあります。**必ず、利用中の OBS バージョンに合った ZIP をダウンロードしてください。**
...
> [!IMPORTANT]
> ダウンロードする ZIP は、OBS のバージョンと一致するものを選んでください。一致しない ZIP を入れると、プラグインが読み込まれないことがあります。
...
> [!WARNING]
> **スレートシーンの音声:** この 2 つのシーンの音声ミキサーに、**マイク** や **デスクトップ音**（ゲーム音など）を含めないでください。視聴者にリアルタイムの音声がそのまま届き、遅延の意味がなくなります。**BGM だけ** を別途設定するのは問題ありません。

4. OBS の **設定 → 出力 → 配信遅延** が有効になっている場合は **オフ** にしてください。DelayDeck と OBS 標準の配信遅延は同時に使えません。

> [!TIP]
> テスト配信で動作を確認してから、本番配信に使ってください。
...
<details>
<summary><strong>Relay Engine を再起動すると配信が切れる？</strong></summary>

<br>

**はい。** DelayDeck は OBS から Relay を経由して配信先へ送出する構成です。トラブルで Relay Engine が停止した場合や、Dock の **エンジンを再起動** で手動再起動した場合、配信先への送出も止まるため、視聴者向けの配信は一旦切断されます。Relay が再接続するまで、Twitch などでは配信終了と扱われることがあります。これはシステム設計上想定された挙動です。

**Twitch で配信している場合**、次の設定で、Relay 再開までの短い切断中も配信が途切れなくなります。

1. [Twitch クリエイターダッシュボード](https://dashboard.twitch.tv/) を開く
2. **設定 → 配信** へ進む
3. **接続切断時の保護** を **有効** にする

Twitch 向けに配信する場合は、可能であれば事前に有効にしておくことをおすすめします。YouTube など他のプラットフォームに同等の設定があるかは、各サービスのドキュメントを確認してください。
...
本リポジトリは [GNU General Public License v2.0 (GPL-2.0)](LICENSE) の条件の下で提供されます。条項の全文は [LICENSE](LICENSE) を参照してください。
