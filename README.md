...
標準機能では、配信中に遅延の有効/無効を切り替えられません。特に時差のないリアルタイム対戦ゲームを配信する場合、視聴者へ情報が先に届き、配信者が不利になることがあります。DelayDeck は、そのような用途向けに開発されました。
...
OBS Studio はバージョンによってプラグインが読み込まれないことがあります。**必ず、利用中の OBS バージョンに合った ZIP をダウンロードしてください。**
...
これで導入は完了です。テスト配信で動作を確認してから、本番配信に使ってください。
...
DelayDeck を使って **長時間の配信（目安: 2 時間以上）** に成功した場合は、ぜひ開発者へご連絡ください。気軽に報告する場合は [X（@KatoJunta）](https://x.com/KatoJunta) がおすすめです。詳細な報告や公開で記録に残したい場合は [GitHub Issues](https://github.com/KatoJunta/DelayDeck/issues) も使えます。
